...
Chat commands
---------------------------------------------------------------------------*/
local CallSecurityTbl = {
	TEAM_SECURITY, 
	TEAM_SECINSTRUCTOR, 
	TEAM_SECURITYADMIN
}

local function CallSecurity(ply, args)
	if args == "" then
	...
	local DoSay = function(text)
		...
		for k, v in pairs(player.GetAll()) do
			if RPExtraTeams[v:Team()] and table.HasValue(CallSecurityTbl, v:Team()) or v == ply then
				GAMEMODE:TalkToPerson(v, team.GetColor(ply:Team()), "(SECURITY REQUEST) " .. ply:Nick(), Color(255, 100, 100, 255), text, ply)
			end
		end
	end
	return args, DoSay
end
AddChatCommand("/cs", CallSecurity, 1.5)
AddChatCommand("/callsecurity", CallSecurity)
