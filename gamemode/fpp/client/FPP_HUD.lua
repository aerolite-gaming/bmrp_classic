...
local function RetrieveOwner(um)
	...
	local function setData(ent)
		...
		if IsValid(ent) and ent:GetClass() == "npc_gman" then
			surface.PlaySound("music/stingers/hl1_stinger_song27.mp3")
			ent.FPPCanTouch = canTouch
		else
			if IsValid(ent) and ( string.find( ( ent:GetClass() ), "npc" ) ) then
