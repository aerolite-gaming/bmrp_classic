...
local function ChangeJob(ply, args)
	...
	if ply.LastJob and 10 - (CurTime() - ply.LastJob) >= 0 and not ply:IsAdmin() then
	...
	if not GAMEMODE.Config.customjobs then
		if ply:IsUserGroup("vip") or ply:IsAdmin() then
			...
			GAMEMODE:Notify(ply, 1, 4, "/job is is a VIP only feature, sorry!")
			return ""
		end	
	end
...
local function Roll(ply)
	if GAMEMODE.Config.alltalk then
		for _, target in pairs(player.GetAll()) do
			GAMEMODE:TalkToPerson(target, team.GetColor(ply:Team()), "*Roll* " .. ply:Nick() .. "has rolled " .. print( math.random( 1, 100 ) ) .. " out of 100.")
		end
	else
		GAMEMODE:TalkToRange(ply, "*Roll* " .. ply:Nick() .. " has rolled " .. math.random( 1, 100 ) .. " out of 100.","", 250)
	end
	return DoSay
end
AddChatCommand("/roll", Roll, 1.5)
...
AddChatCommand("/showid", ShowID, 1.5)
...
local function It(ply, args)
	...
	local DoSay = function(text)
		...
		if GAMEMODE.Config.alltalk then
			...
			GAMEMODE:TalkToRange(ply,  "* " .. text .. " * (" .. ply:Nick() .. ")", "", 250)
		end
	end
	return args, DoSay
end
    ...
    local DoSay = function(text)
		...
		for k,v in pairs(player.GetAll()) do
			if ply:GetPData("disguised") ~= "1" then
                if ply:IsAdmin() then
                    if ply:SteamID() == "STEAM_0:1:6146213" or ply:SteamID() == "STEAM_0:0:38586423" or ply:SteamID() == "STEAM_0:1:24302895" or ply:SteamID() == "STEAM_0:1:66459838" then
						GAMEMODE:TalkToPerson(v, col, "(Developer) (OOC) "..ply:Name(), col2, text, ply)
                    else   
						GAMEMODE:TalkToPerson(v, col, "(Admin) (OOC) "..ply:Name(), col2, text, ply)
                    end    
                    
					else
                        if ply:IsUserGroup("vip") then
                            GAMEMODE:TalkToPerson(v, col, "(VIP) (OOC) "..ply:Name(), col2, text, ply)
                        else
                            if ply:IsUserGroup("moderator") then
                                GAMEMODE:TalkToPerson(v, col, "(Mod) (OOC) "..ply:Name(), col2, text, ply)
                            else
                                if ply:GetPData("trusted") == "1" then
									if toggleooc == 0 then
										GAMEMODE:Notify(ply, 1, 4, string.format("OOC chat has been disabled."))
									else
										GAMEMODE:TalkToPerson(v, col, "(Trusted) (OOC) "..ply:Name(), col2, text, ply)
									end	
                                else
									if toggleooc == 0 then
										GAMEMODE:Notify(ply, 1, 4, string.format("OOC chat has been disabled."))
...
local function LocalOOC(ply, args)
	local DoSay = function(text)
		if text == "" then return "" end
		GAMEMODE:TalkToRange(ply, "(LOOC) " .. ply:Nick(), text, 300)
	end
	return args, DoSay
end
AddChatCommand(".//", LocalOOC)
AddChatCommand("/looc", LocalOOC)
AddChatCommand("/l", LocalOOC)

function IsInLocker(ply)
	local securitylockers1 = ents.FindInBox( Vector(-6257.958496, 250.031250, -297.064758), Vector(-6214.050781, 577.999878, -242.106232) );
	local securitylockers2 = ents.FindInBox( Vector(-5590.032715, 134.031250, -297.064758), Vector(-5965.999512, 177.968750, -241.906387) );
	local securitylockers3 = ents.FindInBox( Vector(-5885.990234, 661.968750, -297.064758), Vector(-5622.002930, 618.059814, -241.869995) );
	local securitylockers4 = ents.FindInBox( Vector(-5506.031250, 545.975159, -297.064758), Vector(-5549.935059, 458.031250, -241.618927) );
	
	for k, v in pairs(securitylockers1) do
		if ( IsValid(v) and v:IsPlayer() and v:GetName() == ply:GetName() and v:Alive() ) then
			return true
		end
	end

	for k, v in pairs(securitylockers2) do
		if ( IsValid(v) and v:IsPlayer() and v:GetName() == ply:GetName() and v:Alive() ) then
			return true
		end
	end	

	for k, v in pairs(securitylockers3) do
		if ( IsValid(v) and v:IsPlayer() and v:GetName() == ply:GetName() and v:Alive() ) then
			return true
		end
	end	

	for k, v in pairs(securitylockers4) do
		if ( IsValid(v) and v:IsPlayer() and v:GetName() == ply:GetName() and v:Alive() ) then
			return true
		end
	end	
...
function IsInSuitPodium(ply)
	
	local suitup = ents.FindInBox( Vector(-6581.967773, 233.112183, -199.800812), Vector(-6511.895508, -121.929749, -293.046753) );
	
	for k, v in pairs(suitup) do
		if ( IsValid(v) and v:IsPlayer() and v:GetName() == ply:GetName() and v:Alive() ) then
			return true
		end
	end
...
local function SuitUp(ply)
	if RPExtraTeams[ply:Team()].science then
		if IsInSuitPodium(ply) then
			if ply:GetPData("suitedup") == "1" then
				GAMEMODE:Notify(ply, 1, 4, string.format("You are no longer wearing your suit."))
				ply:SetModel( ply:GetPData("oldmodel") )
				ply:SetPData("suitedup", "0")
			else
				ply:SetPData("suitedup", "1")
				ply:SetPData("oldmodel", ply:GetModel())
				ply:SetModel( "models/player/SGG/hev_helmet.mdl" )
				GAMEMODE:Notify(ply, 1, 4, string.format("You are now wearing your suit."))
			end
		else
			GAMEMODE:Notify(ply, 1, 4, string.format("You need to be stood in a HEV podium!"))
		end
	else
		if not RPExtraTeams[ply:Team()].security or RPExtraTeams[ply:Team()].science then GAMEMODE:Notify(ply, 1, 4, string.format("Your job can't use this command")) end
		
		if RPExtraTeams[ply:Team()].security then
			if IsInLocker(ply) then
				if ply:GetPData("secsuitedup") == "1" then
					GAMEMODE:Notify(ply, 1, 4, string.format("You are no longer wearing your officer uniform"))
					ply:SetModel( ply:GetPData("oldmodel") )
					...
					ply:SetPData("secsuitedup", "1")
					ply:SetPData("oldmodel", ply:GetModel())
					oldmodel = ply:GetModel()
						
					if string.find(oldmodel,"male_09") then 
						newmodel = "models/player/hdpp/security/male_09.mdl" 
					elseif string.find(oldmodel,"male_06") then 
						newmodel = "models/player/hdpp/security/male_06.mdl"
					elseif string.find(oldmodel,"male_05") then 
						newmodel = "models/player/hdpp/security/male_05.mdl"
					else
						newmodel = "models/player/hdpp/security/barney.mdl"
					end
					ply:SetModel( newmodel )
					GAMEMODE:Notify(ply, 1, 4, string.format("You are now wearing your officer uniform."))
				end
			else
				GAMEMODE:Notify(ply, 1, 4, string.format("You need to be stood in a locker!"))
			end
		end
	end	
end	
AddChatCommand("/suitup", SuitUp)

local function GetVectorz(ply)
	if ply:IsAdmin() then
		ply:ChatPrint(tostring(ply:GetPos()))
	end	
end
AddChatCommand("/getvector", GetVectorz)

local function PlayerAdvertise(ply, args)
	if ply:Team() == TEAM_CITIZEN then
		GAMEMODE:Notify(ply, 1, 4, string.format("Visitors can't make announcements"))
	else
		if args == "" then return "" end
		local DoSay = function(text)
			if text == "" then return end
			GAMEMODE:Notify(ply, 0, 4, string.format("Your announcement has been sent and will be displayed shortly."))
			for k,v in pairs(player.GetAll()) do
				local col = team.GetColor(ply:Team())
				timer.Simple( math.random( 7, 18 ), function() GAMEMODE:TalkToPerson(v, col, LANGUAGE.announcement .." "..ply:Nick(), Color(255,129,0), text, ply)  end )
			end
		end
		return args, DoSay
	end	
...
local function MayorBroadcast(ply, args)
	...
	if not RPExtraTeams[ply:Team()] or not RPExtraTeams[ply:Team()].mayor then 
		GAMEMODE:Notify(ply, 1, 4, "You have to be Administrator.") return "" 
	end
...
local function FireEmployee(ply, args)
	if RPExtraTeams[ply:Team()] and RPExtraTeams[ply:Team()].mayor or RPExtraTeams[ply:Team()] and ply:Team() == TEAM_HEALTHINSPECTOR then
		local tableargs = string.Explode(" ", args)
		if #tableargs == 1 then
			GAMEMODE:Notify(ply, 1, 4, DarkRP.getPhrase("vote_specify_reason"))
			return ""
		...
		reason = string.sub(reason, 2)
		if string.len(reason) > 99 then
			GAMEMODE:Notify(ply, 1, 4, DarkRP.getPhrase("unable", "/fire", "<100"))
			return ""
		end
		...
		if p then
			if p == ply then
				GAMEMODE:Notify(ply, 1, 4, "You can't fire yourself.")
				return ""
			end
			...
			if not PlayerInRange then 
				GAMEMODE:Notify(ply, 1, 4, "You can't fire " .. p:Nick() .. " because you are not near enough!")
				return ""
			end

			if ply:Team() == TEAM_HEALTHINSPECTOR then
				if p:Team() == TEAM_MAYOR or p:Team() == TEAM_HECU or p:Team() == TEAM_HECUMEDIC or p:Team() == TEAM_HECUCOMMANDER then else
					GAMEMODE:Notify(ply, 1, 4, "You can only fire the Administrator or a HECU Unit as Government Worker!")
					return ""		
				end	
			end

			if ply:Team() == TEAM_MAYOR and p:Team() == TEAM_HEALTHINSPECTOR then
				GAMEMODE:Notify(ply, 1, 4, "You can't fire a Government Worker as Administrator!")
				return ""			
			end

			if p:Alive() then
				if p:Team() ~= TEAM_CITIZEN then
					GAMEMODE:TalkToPerson(p, team.GetColor(ply:Team()), "(FIRED) "..ply:Nick(),Color(255,0,0,255), "You have been fired. Reason: "..reason, p)
					GAMEMODE:TalkToPerson(ply, team.GetColor(ply:Team()), "(FIRED) "..ply:Nick(),Color(255,0,0,255), "You have fired '" .. p:Nick() .. "'. Reason: "..reason, p)
					PrintTable(RPExtraTeams[ply:Team()])

					p:TeamBan()
					p:ChangeTeam(TEAM_CITIZEN, true)
				else
					GAMEMODE:Notify(ply, 1, 4, p:Nick().." is not an Employee.")
				end
			else
				GAMEMODE:Notify(ply, 1, 4, "You can't fire a dead Employee!")
			end
			return ""
		else
			GAMEMODE:Notify(ply, 1, 4, DarkRP.getPhrase("could_not_find", "player: "..tostring(args)))
		...
		GAMEMODE:Notify(ply, 1, 4, DarkRP.getPhrase("incorrect_job", "/fire", ""))
	end
	return ""
end
AddChatCommand("/fire", FireEmployee)
