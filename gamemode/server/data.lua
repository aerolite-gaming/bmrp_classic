	...
	MySQLite.begin()
		...
		MySQLite.query([[
			CREATE TABLE IF NOT EXISTS darkrp_door(
				...
				isRammable BOOLEAN NOT NULL DEFAULT FALSE,
				PRIMARY KEY(idx, map)
			);
		]])
	...
	for i = 0, count, 1000 do -- SQLite selecting limit
		MySQLite.query([[SELECT darkrp_wallets.steam, amount, salary, name FROM darkrp_wallets
			...
			if count - i < 1000 then -- the last iteration
				...
				MySQLite.query([[DROP TABLE darkrp_xp;]])
			end
...
function DB.RetrievePlayerData(ply, callback, failed, attempts)
	...
	MySQLite.query("SELECT rpname, wallet, salary, xp FROM darkrp_player WHERE uid = " .. ply:UniqueID() .. ";", callback, function()
		DB.RetrievePlayerData(ply, callback, failed, attempts + 1)
	end)
end

function DB.CreatePlayerData(ply, name, salary, wallet, xp)
...
function DB.StoreXP(ply, XP)
	if not IsValid(ply) then return end
	if XP < 0  then return end

	MySQLite.query([[UPDATE darkrp_player SET xp = ]] .. XP .. [[ WHERE uid = ]] .. ply:UniqueID())
end
...
function DB.RetrieveXP(ply, callback)
	if not IsValid(ply) then return 0 end

	if ply:getDarkRPVar("xp") then return callback and callback(ply:getDarkRPVar("xp")) end -- First check the cache.

	MySQLite.queryValue("SELECT xp FROM darkrp_player WHERE uid = " .. ply:UniqueID() .. ";", function(r)
		local normal = 0
		if not r then
			ply:SetSelfDarkRPVar("xp", normal)
			callback(normal)
		else
			callback(r)
		end
...
function DB.StoreDoorOwnability(ent)
	...
	local nonRammable = ent.DoorData.NonRammable
	...
	MySQLite.query([[REPLACE INTO darkrp_door VALUES(]]..
		...
		"NULL" .. [[, ]] ..
		(ent.DoorData.NonOwnable and 1 or 0) .. [[, ]] .. (ent.DoorData.NonRammable or 0) .. [[);]])
end
...
function setUpNonOwnableDoors()
	MySQLite.query("SELECT idx, title, isLocked, isDisabled, isNonRammable FROM darkrp_door WHERE map = " .. MySQLite.SQLStr(string.lower(game.GetMap())) .. ";", function(r)
		...
		for _, row in pairs(r) do
			...
			if IsValid(e) then
				...
				e.DoorData.NonRammable = row.isNonRammable
