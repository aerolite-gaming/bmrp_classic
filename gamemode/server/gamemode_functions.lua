...
function GM:PlayerSpawnProp(ply, model)
	...
	if ply:isArrested() or ply:Team() == TEAM_CITIZEN and not ply:IsAdmin() then return false end
	if ply:getDarkRPVar("money") <5 and ply:GetPData("disguised") ~= "0" then
		GAMEMODE:Notify(ply, 1, 4, "Need " .. GAMEMODE.Config.currency .. GAMEMODE.Config.propcost .. " to spawn this prop")
		return false
	end
	for id, ent in pairs( ents.FindInSphere( ply:GetPos(), 200 ) ) do
		if ( ent:GetClass() == "func_tracktrain" ) then
			GAMEMODE:Notify(ply, 1, 4, "Cannot spawn props that close to the tram!")
			return false
		end
	end
	...
	local XPAmount = ply:getDarkRPVar("xp")
	local ServerMaxProps = GetConVar("sbox_maxprops"):GetInt()
	local MaxProps = 200
	local PropsXP = 0.25
	if ply:IsUserGroup("vip") or ply:IsUserGroup("moderator") then PropsXP = 0.30 end
	local NewMaximumProps = math.Round(XPAmount / 5 * PropsXP + MaxProps, 0)

	if ply:IsAdmin() then return true end
	if ply:GetCount("props") >= NewMaximumProps or ply:GetCount("props") >= ServerMaxProps then
		GAMEMODE:Notify(ply, 1, 4, "You've hit the Prop Limit!")
		return false
	else
		return true
	end	
...
function GM:PlayerSpawnRagdoll(ply, model)
	local neededRagdollXP = 200
	if ply:getDarkRPVar("xp") < neededRagdollXP then 
		GAMEMODE:Notify(ply, 1, 4, "You need "..neededRagdollXP.." XP to spawn a Ragdoll.")
		return false
	end
...
function GM:PlayerDeath(ply, weapon, killer)
	SendUserMessage("BlackAndWhite", ply, true)
	ply:SetPData("playerdead", "1")
	...
	if ply:isArrested() and not GAMEMODE.Config.respawninjail  then
		...
		ply.NextSpawnTime = CurTime() + math.Clamp(GAMEMODE.Config.respawntime, 0, 30)
	end
	ply.DeathPos = ply:GetPos()

	if GAMEMODE.Config.dropmoneyondeath and ply.pk_pill_ent == nil and ply:FAdmin_GetGlobal("FAdmin_cloaked") ~= true and ply:FAdmin_GetGlobal("FAdmin_godded") ~= true then
...
function GM:PlayerInitialSpawn(ply)
	...
	if ply:IsBot() and botkickervar == true then
		ply:Kick()
	end
	
	ply:ConCommand("DrawPlayerIcon")
	...
	ply:NewData()
	ply.SID = ply:UserID()
	ply:SendLua("deathcause = 'error' ")
	ply:SetPData("atmlockout", 0)
	
	if dbcalledcrapfix ~= 1 then
		FPP.Init()
		DarkRP.initDatabase()
		dbcalledcrapfix = 1
	end	
	...
	if ply:GetPData("FinishedQuiz") ~= "1" then
		ply:ConCommand("Question9")
	end
	
	/* XP System */
	timer.Create(ply:SteamID() .. "XPTimer", 600, 0, function()
		GetXP = ply:getDarkRPVar("xp")
		...
		ply:SetSelfDarkRPVar("xp", GetXP + xpgiveamount )
		
		XP = ply:getDarkRPVar("xp")
		DB.StoreXP(ply, XP)
		ply:ChatPrint("For playing on the server for 10 minutes you have been awarded " ..xpgiveamount.. "XP.")
	end)
...
function GM:PlayerSpawn(ply)
	...
	if ply:GetPData("playerdead") == "1" then
		if ply:Team() ~= TEAM_CITIZEN then
			ply:ChangeTeam( TEAM_CITIZEN, true)
		end
	end
	
	ply:SetPData("playerdead", "0")	
	ply:SetNWInt( "FoodTimeout", 0 )
	ply:SetPData("suitedup", "0")
	ply:SetPData("secsuitedup", "0")
	self.BaseClass:PlayerSpawn(ply)
	ply:ConCommand("RemoveSpawnIcon")
	ply:ConCommand("DrawPlayerIcon")
	...
	SendUserMessage("BlackAndWhite", ply, false)
...
function GM:PlayerDisconnected(ply)
	...
	timer.Destroy(ply:SteamID() .. "XPTimer")
	timer.Destroy(ply:SteamID() .. "ResetATM")
	for k, v in pairs(ents.GetAll()) do
...
function GM:PlayerUse(ply, entity)
	...
	/*if not IsValid(entity) or ply:EyePos():Distance(entity:GetPos()) > 200 then
		return
	end*/
	...
	*/	
	...
	local adminbtns = {
		"playeventnoise",  -- Play Event Noise
		"event1shake",  -- Event 1 Shake
		"eventnoise2",  -- Event Noise 2
		"triggerdisaster",  -- Trigger Disaster
		"stopmachine",  -- Stop Machine
		"earthfire",  -- Earthquake + Fire
		"invasionspawn",  -- Invasion Spawner
		"spawngargantua", -- Spawn Gargantua
		"invasionnoise",  -- Invasion Noise
		"announcementsoff", -- Announcements Off
		"announcementsgoodon", -- Announcements Bad On
		"announcementsgoodenable", -- Announcements Good Enabled
		"shakeexplode",  -- Shake + Explode
		"tramfloortoggle", -- Tram Trigger Hurt
		"outdoor_doors locker", -- Outdoor Entrance Doors Button
		"amsbuttonreset"  -- AMS Button Reset
	}
	
	if table.HasValue(adminbtns, entity:GetName()) then
	...
	*/
	...
	if entity:GetName() == "controlroom scanner 1352" then
		if ply:Team() == TEAM_HECU or ply:Team() == TEAM_HECUCOMMANDER or ply:Team() == TEAM_HECUMEDIC or ply:GetPData("disguised") == "0" then
			for k,v in pairs(ents.FindByName("armydoor")) do v:Fire("unlock", "", 0) end
			return true
		else
			if antispamhecu == 0 then
				timer.Create( "VOXDenyHECU", 0.1, 1, function()
					ply:ChatPrint("Access Denied. HECU only.")
					antispamhecu = 1
					
					timer.Create( "AntiSpamHECU", 3, 1, function()
					antispamhecu = 0
					end)
					
					entity:EmitSound("vox/access.wav", 80, 100)
					timer.Create( "VOXDenyHECU2", 0.8, 1, function()
						entity:EmitSound("vox/denied.wav", 80, 100)
					end)
				end)
			end
			return false
	...
	*/
	...
	if entity:GetName() == "controlroom scanner 1699" or entity:GetName() == "controlroom scanner 169" or entity:GetName() == "outsidedoors" then
		
		if ply:Team() ~= TEAM_CITIZEN or ply:GetPData("disguised") == "0" then
			return true
		else
			if antispam == 0 then
	...
	*/			
	...
	if entity:GetName() == "lambdalab scanner" or entity:GetName() == "lambdalab doors" then
		
		if ply:Team() == TEAM_PHYSICIST or ply:Team() == TEAM_BIOWORKER or ply:Team() == TEAM_MAYOR or ply:Team() == TEAM_BIOLOGIST or ply:Team() == TEAM_HES or ply:Team() == TEAM_SCIINSTRUCTOR or ply:Team() == TEAM_HEADPHYSICIST or ply:Team() == TEAM_SECURITY or ply:Team() == TEAM_SECINSTRUCTOR or ply:Team() == TEAM_SECURITYADMIN or ply:GetPData("disguised") == "0" then
			return true
		else
		
			if antispam2 == 0 then
				timer.Create( "VOXDeny14", 0.1, 1, function()
				ply:ChatPrint("Access Denied. Scientists and Security only.")
				antispam2 = 1
	...
	*/					
	...
	if entity:GetName() == "controlroom scanner 1" or entity:GetName() == "controlroom scanner door" or entity:GetName() == "controlroom scanner 2" or entity:GetName() == "chamber scanner"  then
	...
	*/				
	...
	if entity:GetName() == "lambdalab poweroff" or entity:GetName() == "lambdalab mainbutton 1" or entity:GetName() == "lambdalab mainbutton 2" then
		
		if ply:Team() == TEAM_PHYSICIST or ply:Team() == TEAM_BIOWORKER or ply:Team() == TEAM_MAYOR or ply:Team() == TEAM_BIOLOGIST or ply:Team() == TEAM_HES or ply:Team() == TEAM_SCIINSTRUCTOR or ply:Team() == TEAM_HEADPHYSICIST or ply:GetPData("disguised") == "0" then
			return true
		else
		
			if antispam4 == 0 then
				timer.Create( "VOXDeny12", 0.1, 1, function()
				ply:ChatPrint("Sorry, you don't have the required access.")
				antispam4 = 1
	...
	*/			
	...
	if entity:GetName() == "cartbutton" or entity:GetName() == "keypad1" or entity:GetName() == "secondbutton" then
		...
		if ply:Team() == TEAM_PHYSICIST or ply:Team() == TEAM_BIOWORKER or ply:Team() == TEAM_MAYOR or ply:Team() == TEAM_BIOLOGIST or ply:Team() == TEAM_HES or ply:Team() == TEAM_SCIINSTRUCTOR or ply:Team() == TEAM_HEADPHYSICIST or ply:GetPData("disguised") == "0" then
			...
			if antispam5 == 0 then
				...
				entity:EmitSound("buttons/button2.wav", 80, 100)
	...
	*/			
	...
	if entity:GetName() == "motor_button" then
		
		if ply:Team() == TEAM_EXPLORATION or ply:Team() == TEAM_HEADEXPLORATION or ply:GetPData("disguised") == "0" then
			return true
		else
	...
	*/			
	...
	if entity:GetName() == "controlroom scanner 135"  or entity:GetName() == "lk134" then
	...
	*/					
	...
	if entity:GetName() == "sector c access scanner" then
		
		if ply:Team() == TEAM_PHYSICIST or ply:Team() == TEAM_MAYOR or ply:Team() == TEAM_BIOWORKER or ply:Team() == TEAM_HES or ply:Team() == TEAM_SCIINSTRUCTOR or ply:Team() == TEAM_HEADPHYSICIST or ply:Team() == TEAM_SECURITY or ply:Team() == TEAM_SECINSTRUCTOR or ply:Team() == TEAM_SECURITYADMIN or ply:GetPData("disguised") == "0" then
			return true
		else
	...
	*/			
	...
	if entity:GetName() == "secdoor scanner" then
		
		if ply:Team() == TEAM_SECURITY or ply:Team() == TEAM_SECINSTRUCTOR or ply:Team() == TEAM_SECURITYADMIN or ply:Team() == TEAM_MAYOR or ply:GetPData("disguised") == "0" then
			return true
		else
		
			if antispam8 == 0 then
				timer.Create( "VOXDeny20", 0.1, 1, function()
				ply:ChatPrint("Access Denied. Security only.")
				antispam8 = 1
	...
	*/					
	...
	if entity:GetName() == "carenterlocker" then
		
		if ply:Team() == TEAM_SECURITY or ply:Team() == TEAM_SECINSTRUCTOR or ply:Team() == TEAM_SECURITYADMIN or ply:GetPData("disguised") == "0" then
			return true
		else
		
			if antispam9 == 0 then
				timer.Create( "VOXDeny19", 0.1, 1, function()
				ply:ChatPrint("Access Denied. Security only.")
				antispam9 = 1
					
					timer.Create( "AntiSpam09", 3, 1, function()
						antispam9 = 0
					end)
					
				entity:EmitSound("buttons/button2.wav", 80, 100)
						timer.Create( "VoxDeny29", 0.8, 1, function()
						--trace.Entity:EmitSound("vox/denied.wav", 80, 100)
					end)
				end)
				else 
	...
	if entity.PlayerUse == false then
		return false
	end	
	
	if entity:GetClass() == "func_recharge" then
	...
	/*if entity:GetName() ~= "controlroom scanner 1699" then
		return true
