...
hook.Add("PlayerNoClip", "FAdmin_noclip", function(ply, desiredNoClipState)
	...
	((FAdmin.Access.PlayerHasPrivilege(ply, "Noclip") and not ply:FAdmin_GetGlobal("FADmin_DisableNoclip")) or ply:FAdmin_GetGlobal("FADmin_CanNoclip")) then
		...
		if desiredNoClipState and ply:GetInfoNum("cl_cloakgod_noclip", 0) == 1 then
			if not ply:FAdmin_GetGlobal("FAdmin_cloaked") then Cloak(ply, "", "") end
			if not ply:FAdmin_GetGlobal("FAdmin_godded") then God(ply, "", "") end
		end
