...
if CLIENT then
	function meta:DrawOwnableInfo()
		...
		if whiteText then
			draw.DrawText(st, "UiBold", pos.x + 1, pos.y + 1, Color(0, 0, 0, 200), 1)
			draw.DrawText(st, "UiBold", pos.x, pos.y, Color(255, 255, 255, 200), 1)
		else
			draw.DrawText(st, "UiBold", pos.x , pos.y+1 , Color(0, 0, 0, 255), 1)
			draw.DrawText(st, "UiBold", pos.x, pos.y, Color(128, 30, 30, 255), 1)
		end
	end
...
-- Rammability 
local timeram = false
local function SetDoorRammable(ply)
	if timeram then
		GAMEMODE:Notify(ply, 1, 4, DarkRP.getPhrase("have_to_wait", "0.1", "/togglerammable"))
		return ""
	end
	timeram = true
	timer.Simple(0.1, function() timeram = false end)
	...
	if ent.DoorData.NonRammable == 0 or ent.DoorData.NonRammable == nil then
		GAMEMODE:Notify(ply, 1, 4, "Door is no longer rammable")
		ent.DoorData.NonRammable = 1
	else
		GAMEMODE:Notify(ply, 1, 4, "Door is now rammable")
		ent.DoorData.NonRammable = 0
	end
	-- Save it for future map loads
	DB.StoreDoorOwnability(ent)
	ply.LookingAtDoor = nil -- Send the new data to the client who is looking at the door :D
...
AddChatCommand("/togglerammable", SetDoorRammable)
-- End Rammability 
