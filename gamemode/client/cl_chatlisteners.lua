...
local function chatGetRecipients()
	...
	for _, ply in pairs(player.GetAll()) do
		...
		-- ADDED THIS
		--if ply:GetNoDraw() == true then return false end
		-- ADDED THIS
		
		/*if ply:GetNoDraw() == true then
			if numofcloaked == nil then 
				numofcloaked = 0 
			end
			numofcloaked = numofcloaked + 1 
		end*/	
		
		if not IsValid(ply) or ply:GetNoDraw() == true or ply == LocalPlayer() then continue end
