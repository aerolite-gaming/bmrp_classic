...
local function ChangeJobVGUI()
	if not F4Menu or not F4Menu:IsValid() then
		...
		F4Menu:SetTitle("Control Panel")
		F4Menu.lblTitle:SetFont("F4Title")

		F4Menu.Paint = function(panel, w, h)
			Derma_DrawBackgroundBlur(F4Menu, SysTime())

			surface.SetMaterial(BlurMaterial)
			surface.SetDrawColor(255, 255, 255, 255)

			local x, y = F4Menu:GetPos()

			for i=1, 3 do
				BlurMaterial:SetFloat("$blur", i)
			...
			surface.SetDrawColor(120, 0, 20, 255)
			surface.DrawRect(0, 0, w, 24)
		end

		F4Menu:ShowCloseButton(false)

		GAMEMODE:addF4MenuTab("Money/Actions", GAMEMODE:MoneyTab(), "icon16/money.png")
		GAMEMODE:addF4MenuTab("Jobs", GAMEMODE:JobsTab(), "icon16/user_suit.png")
		GAMEMODE:addF4MenuTab("Information", GAMEMODE:InformationTab(), "icon16/information.png")
		GAMEMODE:addF4MenuTab("Rules", GAMEMODE:RulesTab(), "icon16/delete.png")
		GAMEMODE:addF4MenuTab("Marketplace", GAMEMODE:EntitiesTab(), "icon16/cart.png")
	...
	if not F4MenuTabs or not F4MenuTabs:IsValid() then
		...
		F4MenuTabs:SetPos(5, 29)
		F4MenuTabs:SetSize(F4Menu:GetWide() - 10, F4Menu:GetTall() - 35)
		F4MenuTabs.Paint = function(panel, w, h)
			surface.SetDrawColor(Color(50, 50, 50, 60))
			surface.DrawRect(0, 0, w, h)			
		end
		...
		for k, v in pairs(F4MenuTabs.Items) do
			if not (v.Tab) then continue end

			v.Tab.Paint = function(panel, w, h)
				surface.SetDrawColor(panel:IsActive() and Color(120, 120, 160, 60) or Color(120, 120, 130, 40))
				surface.DrawRect(0, 0, w-6, 24)
			end

			v.Tab.ApplySchemeSettings = function()
				local w, h = v.Tab:GetContentSize()
				v.Tab:SetSize(w + 10, 28)
			end

			v.Tab.PerformLayout = function()
				v.Tab.Image:SetPos(4, 4)
			end
		end
	end
	...
	elseif not trace.Entity:OwnedBy(LocalPlayer()) and trace.Entity:IsOwnable() and not trace.Entity:IsOwned() and not trace.Entity.DoorData.NonOwnable then
		...
		if (FAdmin and FAdmin.Access.PlayerHasPrivilege(LocalPlayer(), "rp_doorManipulation") or LocalPlayer():IsAdmin()) then
			if trace.Entity.DoorData.GroupOwn then
				...
				Frame:SetSize(200, 555)
			end

			local DisableRamming = vgui.Create("DButton", Frame)
			DisableRamming:SetPos(10, Frame:GetTall() - 420)
			DisableRamming:SetSize(180, 100)
			DisableRamming:SetText("Toggle Rammability")
			DisableRamming.DoClick = function() Frame:Close() RunConsoleCommand("darkrp", "/togglerammable") end

			local CombineStatus = vgui.Create("DButton", Frame)
			CombineStatus:SetPos(10, Frame:GetTall() - 315)
			CombineStatus:SetSize(180, 100)
			CombineStatus:SetText("Toggle Combine Mode")
			CombineStatus.DoClick = function() Frame:Close() RunConsoleCommand("darkrp", "/togglecombine") end
			
			local DisableOwnage = vgui.Create("DButton", Frame)
			DisableOwnage:SetPos(10, Frame:GetTall() - 210)
			...
			local SetCopsOnly = vgui.Create("DButton", Frame)
			SetCopsOnly:SetPos(10, Frame:GetTall() - 105)
			SetCopsOnly:SetSize(180, 100)
			SetCopsOnly:SetText("Edit Door Group")
			SetCopsOnly.DoClick = function()
				local menu = DermaMenu()
