...
function SWEP:PrimaryAttack()
	...
	if trace.Entity:IsDoor() and trace.Entity.DoorData.NonRammable == 1 then
		GAMEMODE:Notify(self.Owner, 1, 5, "This door cannot be forced open")
		return
	end	
...
function SWEP:SecondaryAttack()
	...
	if (self.Owner:getDarkRPVar("Energy") == 0) and (self:Health() > 1) then
		self.RS = 100
	else
		self.RS = 240
	end
	if self.Ready then
		...
		if SERVER then
			hook.Call("UpdatePlayerSpeed", GAMEMODE, self.Owner)
			self.Owner:SetJumpPower(200)
			self.Owner:SetRunSpeed(self.RS)
		end
	end
end
