...
function SWEP:PrimaryAttack()
	...
	if not IsValid(trace.Entity) or not trace.Entity:IsOwnable() or (trace.Entity.DoorData and trace.Entity.DoorData.NonOwnable) or (trace.Entity:IsDoor() and self.Owner:EyePos():Distance(trace.Entity:GetPos()) > 65) or (trace.Entity:IsVehicle() and self.Owner:EyePos():Distance(trace.Entity:NearestPoint(self.Owner:EyePos())) > 100) then
		if CLIENT then 
			if LocalPlayer():Team() == TEAM_PHYSICIST or LocalPlayer():Team() == TEAM_BIOWORKER or LocalPlayer():Team() == TEAM_BIOLOGIST or LocalPlayer():Team() == TEAM_HES or LocalPlayer():Team() == TEAM_SCIINSTRUCTOR or LocalPlayer():Team() == TEAM_EXPLORATION or LocalPlayer():Team() == TEAM_HEADEXPLORATION or LocalPlayer():Team() == TEAM_HEADPHYSICIST then
				RunConsoleCommand("SciencePhrases") 
			end
			
			if LocalPlayer():Team() == TEAM_SECURITY or LocalPlayer():Team() == TEAM_SECINSTRUCTOR or LocalPlayer():Team() == TEAM_SECURITYADMIN then
				RunConsoleCommand("SecurityPhrases") 
			end

			if LocalPlayer():Team() == TEAM_HECU or LocalPlayer():Team() == TEAM_HECUCOMMANDER then
				RunConsoleCommand("HECUPhrases")
			end
		end
		return
	end
	...
	if trace.Entity:OwnedBy(self.Owner) or (DoorData.GroupOwn and table.HasValue(RPExtraTeamDoors[DoorData.GroupOwn] or {}, Team)) or (DoorData.TeamOwn and DoorData.TeamOwn[Team]) then
		if SERVER then
			...
			if self.Owner.LastJob and 120 - (CurTime() - self.Owner.LastJob) >= 0 and self.Owner:GetPData("disguised") ~= "0" then
				GAMEMODE:Notify(self.Owner, 1, 4, string.format("Can't lock until you've had your job for at least 2 minutes."))
				return false
			end
...
function SWEP:SecondaryAttack()
	...
	if trace.Entity:OwnedBy(self.Owner) or (DoorData.GroupOwn and table.HasValue(RPExtraTeamDoors[DoorData.GroupOwn] or {}, Team)) or (DoorData.TeamOwn and DoorData.TeamOwn[Team]) then
		if SERVER then
			...
			if self.Owner.LastJob and 120 - (CurTime() - self.Owner.LastJob) >= 0 and self.Owner:GetPData("disguised") ~= "0" then
				GAMEMODE:Notify(self.Owner, 1, 4, string.format("Can't unlock until you've had your job for at least 2 minutes."))
				return false
			end
